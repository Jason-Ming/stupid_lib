...
PRIVATE _VOID s_cproc_text_release_node(STRU_C_TEXT_NODE **pp_c_text_node)
{
    ...
    S_FREE((*pp_c_text_node)->data.p_filename);
    S_FREE((*pp_c_text_node)->data.p_buffer);
    S_FREE((*pp_c_text_node));
}
    ...
    const _S8 *p_filename)
{
    ...
    *pp_c_text_node = NULL;
    ...
    p_c_text_node->data.inode = s_get_inode(p_filename);
