...
PRIVATE ENUM_RETURN s_cproc_text_add_node_to_list(
    STRU_C_TEXT_NODE *p_c_text_node)
{
    S_R_ASSERT(p_c_text_node != NULL, RETURN_FAILURE);

    list_add_tail(&(p_c_text_node->list), &(g_c_text_list_head.list));
...
ENUM_RETURN s_cproc_text_read_file_to_buffer(const _S8 *p_filename)
{
    ...
    ret_val = s_cproc_text_make_new_node(&p_c_text_node_new, p_filename, inode, p_text_buffer, text_buffer_size);
...
const _S8 * s_cproc_text_get_buffer_by_filename(const _S8 *p_filename)
{
    ...
    list_for_each(pos, &g_c_text_list_head.list)
    {
...
ENUM_RETURN s_cproc_text_release_all_buffer(_VOID)
{
    ...
    struct list_head *pos, *next;
    list_for_each_safe(pos, next, &g_c_text_list_head.list)
    {
