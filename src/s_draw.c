...
{   
    ...
    for(int i = 0; i < table_row; i++)
    {
        ...
        {
            ret_val = set_print_table_value(i, j, PRINT_TYPE_NO, SI_INVALID, NULL);
    ...
    for(int i = 0; i < table_row - 2; i++)
    {
        ...
        R_ASSERT(ret_val == RETURN_SUCCESS, RETURN_FAILURE);
        ret_val = set_print_table_value(i, 1, PRINT_TYPE_VERTICAL_LINE, SI_INVALID, NULL);
        R_ASSERT(ret_val == RETURN_SUCCESS, RETURN_FAILURE);
    }
    ...
    for(int i = 0; i < table_row -2; i++)
    {
        for(int j = 2; j < table_col; j++)
        {
            ...
            {
                ret_val = set_print_table_value(i, j, PRINT_TYPE_YES, SI_INVALID, NULL);
                R_ASSERT(ret_val == RETURN_SUCCESS, RETURN_FAILURE);
            }
        }
    }
    ...
    {
        ret_val = set_print_table_value(table_row - 2, i, PRINT_TYPE_HORIZONTAL_LINE, SI_INVALID, NULL);
        R_ASSERT(ret_val == RETURN_SUCCESS, RETURN_FAILURE);
        
        ret_val = set_print_table_value(table_row - 1, i, PRINT_TYPE_COL_LABEL, SI_INVALID, array[i-2].info);
        R_ASSERT(ret_val == RETURN_SUCCESS, RETURN_FAILURE);
    }

    ret_val = set_print_table_value(table_row - 2, 1, PRINT_TYPE_COL_LABEL, SI_INVALID, "  +--");
    R_ASSERT(ret_val == RETURN_SUCCESS, RETURN_FAILURE);
